...
import { canAccessMeeting, isMeetingHost, requireApiUser } from '../../lib/auth';
...
export async function GET(_req: NextRequest, { params }: { params: { meetingId: string } }) {
  ...
  if (!(await canAccessMeeting(db, auth.user.id, params.meetingId))) {
...
export async function PATCH(request: NextRequest, { params }: { params: { meetingId: string } }) {
  ...
  const auth = await requireApiUser(db);
  if (auth.response) return auth.response;
  if (!(await isMeetingHost(db, auth.user.id, params.meetingId))) {
    return NextResponse.json({ error: 'Meeting not found' }, { status: 404 });
  }
  ...
  const { data, error } = await db.from('meetings').update(updates)
    .eq('id', params.meetingId).eq('host_id', auth.user.id).select('*, meeting_participants(*)').single();
  if (error || !data) return NextResponse.json({ error: 'Meeting not found' }, { status: 404 });
  const { meeting_participants, ...meeting } = data;
  return NextResponse.json({
    ...toCamel(meeting),
...
export async function DELETE(_req: NextRequest, { params }: { params: { meetingId: string } }) {
  ...
  const { error } = await db.from('meetings').delete().eq('id', params.meetingId).eq('host_id', auth.user.id);
